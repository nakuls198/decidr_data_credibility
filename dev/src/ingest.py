...
Reads the Northfield Phase 1 (a plain folder) and Phase 2 (folder or
.zip, as Decidr shipped it) release packs and turns every file into
Documents and Chunks (see schema.py), regardless of whether it is
markdown or CSV.
...
def iter_source_files(root: str) -> Iterable[Tuple[str, str]]:
    ...
    `root` may be a directory or a .zip file. Phase 2 in this studio is
    the extracted folder at data/phase2/. Mac resource-fork junk
    (__MACOSX/, "._*") is skipped automatically.
    """
    root_path = Path(root)
...
def main():
    ...
    ap.add_argument("--phase2", help="Path to phase2_model_development/ (folder or .zip)")
